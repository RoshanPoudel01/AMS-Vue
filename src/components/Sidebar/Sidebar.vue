...
<script>
import { Home, LucidePersonStanding, User } from "lucide-vue-next";
export default {
  ...
  data() {
    return {
      ...
      navLinks: [
        ...
        { name: "Artists", route: "/artists", icon: LucidePersonStanding },
      ],
    };
  },
  methods: {
  ...
  watch: {
    // Watch for route changes to update active page
    $route(to) {
      const path = to.path;
      if (path === "/") {
        this.currentPage = "dashboard";
