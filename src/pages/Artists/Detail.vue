...
import { GetArtistById, UpdateArtist } from "@/services/service-artist";
...
const { mutateAsync: updateArtist, isPending } = UpdateArtist();
...
watch(
  ...
  (newData) => {
    if (newData) {
      ...
      setValues({
        name: newData.artist.name,
        dob: newData.artist.dob,
        first_release_year: newData.artist.first_release_year,
        no_of_albums_released: newData.artist.no_of_albums_released,
        address: newData.artist.address,
        gender: newData.artist.gender.toLowerCase(),
      });
    }
  },
  {
